	...
	for (lc = 0, p = buf; fgets(buf, sizeof(buf), fd) != (void *)0; lc++) {
		...
		if ((char *)strchr(buf, '/') && lc == 0) {
			for (i = 0; *p != '-'; i++, p++)
				addrstr[i] = *p;
			addrstr[i] = '\0';
			addrspace[TEXT_SPACE].svaddr = strtoul(addrstr, (void *)0, 16);
			for (p = p + 1, i = 0; *p != 0x20; i++, p++)
				addrstr[i] = *p;
			addrstr[i] = '\0';
			addrspace[TEXT_SPACE].evaddr = strtoul(addrstr, (void *)0, 16);
		...
		if (strstr(buf, "[heap]")) {
			for (i = 0, p = buf; *p != '-'; i++, p++)
				addrstr[i] = *p;
			addrstr[i] = '\0';
			addrspace[HEAP_SPACE].svaddr = strtoul(addrstr, (void *)0, 16);
			for (p = p + 1, i = 0; *p != 0x20; i++, p++)
				addrstr[i] = *p;
			addrstr[i] = '\0';
			addrspace[HEAP_SPACE].evaddr = strtoul(addrstr, (void *)0, 16);
...
int validate_em_type(char *path)
{
	...
	mem = mmap((void *)0, 4096, PROT_READ, MAP_PRIVATE, fd, 0);
	if (mem == MAP_FAILED) {
		perror("mmap");
		exit(-1);
	}
